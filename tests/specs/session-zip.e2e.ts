...
import { setValueVueInput, mediview-3dPage } from '../pageobjects/mediview-3d.page';
...
const saveSession = async () => {
  const sessionFileName = await mediview-3dPage.saveSession();
...
const annotate = async () => {
  await mediview-3dPage.open();
  await mediview-3dPage.downloadProstateSample();
  await mediview-3dPage.waitForViews();

  // draw rectangle
  await mediview-3dPage.activateRectangle();
  const view = await mediview-3dPage.viewTwoContainer;
...
describe('mediview-3d config and deserialization', () => {
  it('config.json files should override label props of existing tool', async () => {
    ...
    await mediview-3dPage.open(sessionZipAndConfig);
  ...
  it('edited default label is serialized and deserialized', async () => {
    ...
    const buttons = await mediview-3dPage.editLabelButtons;
    await buttons[2].click();
    const input = await mediview-3dPage.labelStrokeWidthInput;
    await setValueVueInput(input, editedStrokeWidth.toString());

    const done = await mediview-3dPage.editLabelModalDoneButton;
    await done.click();

    const { session } = await saveGetManifest();

    const sessionZip = `?urls=[tmp/${session}]`;
    await mediview-3dPage.open(sessionZip);
    await mediview-3dPage.waitForViews();
